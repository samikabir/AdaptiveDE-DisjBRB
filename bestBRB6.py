...
def deRuleBase(s,c,best): 
    ...
    temp_deConsequentBeliefDegree = [best[0], best[1], best[2], best[3], best[4], best[5], best[6], best[7], best[8], best[9], best[10], best[11], best[12], best[13], best[14], best[15], best[16], best[17]] 
    for u in range(18):   
    ...
    print("Inside deRuleBase(best) relativeWeight1 ",best[18],"relativeWeight2 ",best[19])  
     
    H_PM = 0 + (best[26] * 499.4)
    UM_PM = 0 + (best[27] * 499.4)     
    M_PM = 0 + (best[28] * 499.4)
    LM_PM = 0 + (best[29] * 499.4)
    SL_PM = 0 + (best[30] * 499.4)
    L_PM = 0 + (best[31] * 499.4) 
         
    transformInput1(s,H_PM,UM_PM,M_PM,LM_PM,SL_PM,L_PM)      
    transformInput2(c,H_PM,UM_PM,M_PM,LM_PM,SL_PM,L_PM)    
    calculateMatchingDegreeBrbCnn(best[18], best[19], best[20], best[21], best[22], best[23], best[24], best[25]) 
    showActivationWeight()  
    updateBeliefDegree()       
    deResult = aggregateER_BrbCnn()   
    fo = open("de_best.txt", "w") 
      
    for w_cbd in range(18):
        s_cbd = str(consequentBeliefDegree[w_cbd])   
        fo.write(s_cbd)  
        fo.write("\n") 
  
    for w_cmd in range(18, 26):    
