...
def deRuleBase(s,c,best):  
    ...
    temp_deConsequentBeliefDegree = [best[0], best[1], best[2], best[3], best[4], best[5]]  
    for u in range(6):    
    ...
    print("Inside deRuleBase(best) relativeWeight1 ",best[6],"relativeWeight2 ",best[7])   
      
    H_PM = 0 + (best[10] * 499.4)     #500.4     
    #M_PM = 0 + (best[12] * 499.4)     #35.5
    L_PM = 0 + (best[11] * 499.4)     #0     
    transformInput1(s,H_PM,L_PM)      
    transformInput2(c,H_PM,L_PM)    
    calculateMatchingDegreeBrbCnn(best[6], best[7], best[8], best[9]) 
    showActivationWeight()  
    updateBeliefDegree()               
    deResult = aggregateER_BrbCnn()   
    fo = open("de_best.txt", "w") 
     
    for w_cbd in range(6):
        s_cbd = str(consequentBeliefDegree[w_cbd])   
        fo.write(s_cbd)  
        fo.write("\n") 
  
    for w_cmd in range(6, 10):    
