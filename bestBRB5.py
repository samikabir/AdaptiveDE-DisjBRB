...
def deRuleBase(s,c,best):
    ...
    temp_deConsequentBeliefDegree = [best[0], best[1], best[2], best[3], best[4], best[5], best[6], best[7], best[8], best[9], best[10], best[11], best[12], best[13], best[14]] 
    for u in range(15):     
    ...
    consequentBeliefDegree = [de0, de1, de2, de3, de4, de5, de6, de7, de8, de9, de10, de11, de12, de13, de14]  
    ...
    print("Inside deRuleBase(best) relativeWeight1 ",best[15],"relativeWeight2 ",best[16])  
      
    H_PM = 0 + (best[22] * 499.4)
    UM_PM = 0 + (best[23] * 499.4)     
    M_PM = 0 + (best[24] * 499.4)
    LM_PM = 0 + (best[25] * 499.4)
    L_PM = 0 + (best[26] * 499.4)     
     
    transformInput1(s,H_PM,UM_PM,M_PM,LM_PM,L_PM)      
    transformInput2(c,H_PM,UM_PM,M_PM,LM_PM,L_PM)     
    calculateMatchingDegreeBrbCnn(best[15], best[16], best[17], best[18], best[19], best[20], best[21])   
    showActivationWeight()   
    updateBeliefDegree()       
    deResult = aggregateER_BrbCnn()   
    fo = open("de_best.txt", "w") 
     
    for w_cbd in range(15):
        s_cbd = str(consequentBeliefDegree[w_cbd])   
        fo.write(s_cbd)  
        fo.write("\n") 
  
    for w_cmd in range(15, 22):    
...
def calculateMatchingDegreeBrbCnn(aw1,aw2,irw1,irw2,irw3,irw4,irw5):     
    antattrw1 = aw1 
    antattrw2 = aw2
    global initialRuleWeight      
    initialRuleWeight = [irw1, irw2, irw3, irw4, irw5]        
    increment = 0     
    global matchingDegree 
    matchingDegree = [1.51, 1.51, 1.51, 1.51, 1.51]     
     
    global trainedMatchingDegree
    trainedMatchingDegree = [1.51, 1.51, 1.51, 1.51, 1.51]  
...
def showActivationWeight():   
    ...
    activationWeight = [1.51, 1.41, 1.45, 1.51, 1.51]         
    temp_activationWeight = [1.57, 1.81, 1.92, 1.51, 1.51]    
    for x in range(5):    
...
def aggregateER_BrbCnn():    
    ...
    ruleWiseBeliefDegreeSum = [1.51, 1.51, 1.51, 1.51, 1.51]  
