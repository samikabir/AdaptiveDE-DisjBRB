...
def deRuleBase(s,c,best):  
    ...
    print("Inside deRuleBase(best) relativeWeight1 ",best[12],"relativeWeight2 ",best[13])  
     
    H_PM = 0 + (best[18] * 499.4)     #500.4     
    UM_PM = 0 + (best[19] * 499.4)
    M_PM = 0 + (best[20] * 499.4)     #35.5
    L_PM = 0 + (best[21] * 499.4)     #0    
    ...
    for w_cbd in range(12):
        s_cbd = str(consequentBeliefDegree[w_cbd])   
        fo.write(s_cbd)  
        fo.write("\n") 
  
    for w_cmd in range(12, 18):    
...
def showActivationWeight():   
    ...
    activationWeight = [1.51, 1.41, 1.45, 1.51]       
    temp_activationWeight = [1.57, 1.81, 1.92, 1.51]    
    for x in range(4):    
